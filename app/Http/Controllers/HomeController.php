...
class HomeController extends Controller
{
  ...
  public function getVerifyAccount($confirmation_code)
  {
    if (
      ...
      if ($user) {
        ...
        // Send Email Wecome Message
        if ($this->settings->send_welcome_email_new_users) {
          $user->notify(new WelcomeMessageRegistered);
        }
  ...
  public function category($slug, $type = false)
  {
    ...
    if ($type == 'free') {
      ...
      if (!in_array($type, ['featured', 'more-active', 'new', 'free'])) {
        $data->leftjoin('subscriptions', 'subscriptions.creator_id', '=', 'users.id');
