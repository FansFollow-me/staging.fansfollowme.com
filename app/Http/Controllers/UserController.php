...
use App\Jobs\AvatarCoverModeration;
...
class UserController extends Controller
{
  ...
  public function profile($slug, $media = null)
  {
    ...
    abort_if($media && $user->verified_id != 'yes', 404);

    abort_if($media === 'audio' && $this->settings->disable_audio, 404);
    ...
    if (request('media') == 'reels' && auth()->check()) {
      $reels = $user->allReels()
    ...
    if (request('media') == 'reels' && auth()->guest()) {
    ...
    return view('users.profile', [
      ...
      'totalReels' => $user->allReels()->count(),
      'reels' => $reels ?? null,
      '_stripe' => $_stripe,
  ...
  public function settingsNotifications()
  {
    ...
    $user->notify_commented_reel = $this->request->notify_commented_reel;
    $user->notify_liked_reel = $this->request->notify_liked_reel;
    $user->email_upcoming_renewals = $this->request->email_upcoming_renewals;
    $user->save();
  ...
  public function saveSubscription()
  {
    ...
    if (!$this->request->free_subscription) {
      ...
      if ($this->request->price_weekly) {
        Plans::updateOrCreate(
          [
            'user_id' => auth()->id(),
            'name' => 'plan_user_' . auth()->id() . '_weekly'
          ],
          [
      ...
      if ($this->request->price_quarterly) {
        Plans::updateOrCreate(
          [
            'user_id' => auth()->id(),
            'name' => 'plan_user_' . auth()->id() . '_quarterly'
          ],
          [
      ...
      if ($this->request->price_biannually) {
        Plans::updateOrCreate(
          [
            'user_id' => auth()->id(),
            'name' => 'plan_user_' . auth()->id() . '_biannually'
          ],
          [
      ...
      if ($this->request->price_yearly) {
        Plans::updateOrCreate(
          [
            'user_id' => auth()->id(),
            'name' => 'plan_user_' . auth()->id() . '_yearly'
          ],
          [
    ...
    $freeSubscription != auth()->user()->free_subscription
  ...
  public function uploadAvatar()
  {
    $validator = Validator::make($this->request->all(), [
      'avatar' => 'required|mimes:jpg,gif,png,jpe,jpeg,webp|dimensions:min_width=200,min_height=200|max:' . $this->settings->file_size_allowed . '',
    ]);
    ...
    if ($this->request->hasFile('avatar')) {
      ...
      if ($this->settings->moderation_status) {
        dispatch(new AvatarCoverModeration(auth()->user(), 'avatar'));
      }
  ...
  public function uploadCover()
  {
    ...
    if ($this->request->hasFile('image')) {
      ...
      if ($this->settings->moderation_status) {
        dispatch(new AvatarCoverModeration(auth()->user(), 'cover'));
      }
  ...
  public function like(Request $request)
  {
    ...
    $totalLikes = number_format($user->likes()->count() + $user->likes_extras);
  ...
  public function ajaxNotifications()
  {
    if (request()->ajax()) {
      ...
      // Notifications
      $notificationsCount = auth()->user()->notifications()->where('status', '0')->count();
      // Messages
      $messagesCount = auth()->user()->messagesInbox();
      // Video Calls Incoming
      $videoCallsIncoming = auth()->user()->videoCallIncoming()->first();
      // Video Calls Outgoing
      $videoCallsOutgoing = auth()->user()->videoCallOutgoing()->first();
      // Video Call Unanswered
      if ($videoCallsIncoming && $videoCallsIncoming->canBeCanceledAsUnanswered()) {
        $videoCallsIncoming->update(['status' => VideoCallStatus::UNANSWERED]);
      }

      // Audio Calls Incoming
      $audioCallsIncoming = auth()->user()->audioCallIncoming()->first();
      // Audio Calls Outgoing
      $audioCallsOutgoing = auth()->user()->audioCallOutgoing()->first();
      // Audio Call Unanswered
      if ($audioCallsIncoming && $audioCallsIncoming->canBeCanceledAsUnanswered()) {
        $audioCallsIncoming->update(['status' => AudioCallStatus::UNANSWERED]);
      }

      return response()->json([
        'messages' => $messagesCount,
        'notifications' => $notificationsCount,
        'videoCallsIncoming' => $videoCallsIncoming ? [
          'video_call_id' => $videoCallsIncoming->id,
          'status' => $videoCallsIncoming->status,
          'callerAmount' => Helper::formatPrice($videoCallsIncoming->price, true) . ' ' . __('general.per_minutes', ['minutes' => $videoCallsIncoming->minutes]),
          'url' => route('video.call.url', $videoCallsIncoming->token),
          'seller' => $videoCallsIncoming->seller->id,
          'avatar_seller' => Helper::getFile(config('path.avatar') . $videoCallsIncoming->seller->avatar),
          'username_seller' => '@' . $videoCallsIncoming->seller->username
        ] : null,
        'videoCallsOutgoing' => $videoCallsOutgoing ? [
          'video_call_id' => $videoCallsOutgoing->id,
          'status' => $videoCallsOutgoing->status,
          'url' => route('video.call.url', $videoCallsOutgoing->token),
        ] : null,
        'audioCallsIncoming' => $audioCallsIncoming ? [
          'audio_call_id' => $audioCallsIncoming->id,
          'status' => $audioCallsIncoming->status,
          'callerAmount' => Helper::formatPrice($audioCallsIncoming->price, true) . ' ' . __('general.per_minutes', ['minutes' => $audioCallsIncoming->minutes]),
          'url' => route('audio.call.url', $audioCallsIncoming->token),
          'seller' => $audioCallsIncoming->seller->id,
          'avatar_seller' => Helper::getFile(config('path.avatar') . $audioCallsIncoming->seller->avatar),
          'username_seller' => '@' . $audioCallsIncoming->seller->username
        ] : null,
        'audioCallsOutgoing' => $audioCallsOutgoing ? [
          'audio_call_id' => $audioCallsOutgoing->id,
          'status' => $audioCallsOutgoing->status,
          'url' => route('audio.call.url', $audioCallsOutgoing->token),
        ] : null
      ]);
    } else {
  ...
  public function addUpdatePaymentCard()
  {
    ...
    $user->updateDefaultPaymentMethod($this->request->payment_method);


    foreach ($user->paymentMethods() as $paymentMethod) {
      if ($paymentMethod->id !== $this->request->payment_method) {
  ...
  public function downloadFile($id)
  {
    ...
    $fileName = $media->file_name . ' ' . __('general.by') . ' @' . $post->user()->username . '.zip';

    return response()->stream(function () use ($fullPath) {
      $stream = fopen($fullPath, 'rb');
      while (!feof($stream)) {
        echo fread($stream, 8192); // Reads 8KB at a time
        flush();
  ...
  public function savePrivacySecurity()
  {
    ...
    if ($this->settings->addon_crowdfund) {
      $user->display_list_donors = $this->request->display_list_donors ?? false;
    }
  ...
  public function logoutOtherDevices()
  {
    ...
    try {
      ...
      return back()->withErrors([
        'errors' => __('general.password_incorrect'),
      ]);
    }
    ...
    $current = LoginSessions::whereUserId(auth()->id())
      ->where('ip', $ip)
      ->where('device', $device)
      ->where('device_type', $deviceType)
      ->where('browser', $browser)
  ...
  {
    if (auth()->user()->verified_id != 'yes' || $this->settings->live_streaming_private != 'on') {
      abort(404);
    }
  ...
  public function verifyAccountSendAutomatically()
  {
    auth()->user()->update([
      'verified_id' => 'yes'
    ]);
  ...
  public function videoCallSettings()
  {
    abort_if(auth()->user()->verified_id != 'yes' || !config('settings.video_call_status'), 404);

    return view('users.video-call-settings');
  }

  public function storeVideoCallSettings()
  {
    $messages = [
      'price_video_call.required' => __('validation.required', ['attribute' => __('general.price_video_call')]),
      'price_video_call.min' => __('validation.min.numeric', ['attribute' => __('general.price_video_call'), 'min' => Helper::priceWithoutFormat($this->settings->video_call_min_price)]),
      'price_video_call.max' => __('validation.max.numeric', ['attribute' => __('general.price_video_call'), 'max' => Helper::priceWithoutFormat($this->settings->video_call_max_price)]),
    ];

    $this->request->validate([
      'price_video_call' => 'required|numeric|min:' . $this->settings->video_call_min_price . '|max:' . $this->settings->video_call_max_price,
    ], $messages);

    auth()->user()->update([
      'price_video_call' => $this->request->price_video_call,
      'video_call_duration' => $this->request->video_call_duration
    ]);
  ...
  {
    abort_if(auth()->user()->verified_id != 'yes', 404);

    $reels = Reel::whereUserId(auth()->id())
      ->when(request('sort') == 'views', function ($sort) {
        $sort->orderBy('views', 'desc');
      })
      ->when(request('sort') == 'popular', function ($sort) {
        $sort->orderBy('likes', 'desc');
      })
      ->when(request('sort') == 'oldest', function ($sort) {
        $sort->orderBy('id', 'asc');
      })
      ->when(request('q') && strlen(request('q')) >= 3, function ($query) {
    ...
    return view('users.my-reels')->withReels($reels);
  }

  public function audioCallSettings()
  {
    abort_if(auth()->user()->verified_id != 'yes' || !config('settings.audio_call_status'), 404);

    return view('users.audio-call-settings');
  }

  public function storeAudioCallSettings()
  {
    $messages = [
      'price_audio_call.required' => __('validation.required', ['attribute' => __('general.price_audio_call')]),
      'price_audio_call.min' => __('validation.min.numeric', ['attribute' => __('general.price_audio_call'), 'min' => Helper::priceWithoutFormat($this->settings->audio_call_min_price)]),
      'price_audio_call.max' => __('validation.max.numeric', ['attribute' => __('general.price_audio_call'), 'max' => Helper::priceWithoutFormat($this->settings->audio_call_max_price)]),
    ];

    $this->request->validate([
      'price_audio_call' => 'required|numeric|min:' . $this->settings->audio_call_min_price . '|max:' . $this->settings->audio_call_max_price,
    ], $messages);

    auth()->user()->update([
      'price_audio_call' => $this->request->price_audio_call,
      'audio_call_duration' => $this->request->audio_call_duration
    ]);
