...
class AdminController extends Controller
{
	...
	public function admin()
	{
		...
		$earningsChart = Transactions::selectRaw('DATE(`created_at`) as `date`')
			...
			->where('created_at', '<', Carbon::parse('now')->endOfMonth())
			->groupBy('date')
			->orderBy('date', 'ASC')
			->get()
			->pluck('total', 'date');
		...
		$total_subscriptions = Subscriptions::selectRaw('COUNT(id) as total')->whereFree('no')->pluck('total')->first();
		...
		$revenues = Transactions::whereApproved('1')
			...
			->selectRaw('SUM(CASE WHEN created_at BETWEEN "' . Carbon::parse('now')->startOfWeek() . '" AND "' . Carbon::parse('now')->endOfWeek() . '" THEN earning_net_admin ELSE 0 END) AS week')
			->selectRaw('SUM(CASE WHEN created_at BETWEEN "' . Carbon::parse('now')->startOfWeek()->subWeek() . '" AND "' . Carbon::parse('now')->subWeek()->endOfWeek() . '" THEN earning_net_admin ELSE 0 END) AS lastWeek')
			->selectRaw('SUM(CASE WHEN created_at BETWEEN "' . Carbon::parse('now')->startOfMonth() . '" AND "' . Carbon::parse('now')->endOfMonth() . '" THEN earning_net_admin ELSE 0 END) AS month')
			->selectRaw('SUM(CASE WHEN created_at BETWEEN "' . Carbon::parse('now')->startOfMonth()->subMonth() . '" AND "' . Carbon::parse('now')->subMonth()->endOfMonth() . '" THEN earning_net_admin ELSE 0 END) AS lastMonth')
			->first();
	...
	public function update($id, Request $request)
	{
		...
		$user->custom_profit_referral = $request->custom_profit_referral;
		$user->custom_fee = $request->custom_fee;
		$user->featured = $request->featured ?? 'no';
		$user->featured_date = $featured_date;
		$user->balance = $request->balance;
		$user->wallet = $request->wallet;
		$user->age_verification = $request->age_verification;
		$user->save();
	...
	public function saveSettings(Request $request)
	{
		...
		$sql->delete_old_messages = $request->delete_old_messages;
		$sql->allow_reels = $request->allow_reels;
		$sql->disable_audio = $request->disable_audio;
		$sql->ppv_only_free_accounts = $request->ppv_only_free_accounts;
		$sql->allow_vault = $request->allow_vault;
		$sql->send_welcome_email_new_users = $request->send_welcome_email_new_users;
		$sql->allow_delete_account = $request->allow_delete_account;

		if ($this->settings->addon_crowdfund) {
			$sql->allow_crowdfund = $request->allow_crowdfund;
		}
	...
	{
		$data = Subscriptions::with(['subscriber:id,username,avatar,name', 'creator:id,username,name,plan'])
			->whereFree('no')
			->orderBy('id', 'DESC')
			->paginate(50);
	...
	public function transactions(Request $request)
	{
		...
		if (strlen($query) >= 2) {
			$transactions->where('txn_id', 'LIKE', '%' . $query . '%')
				->orWhereHas('creator', function ($q) use ($query) {
					$q->where('username', 'like', "%$query%")
						->orWhere('name', 'like', "%$query%");
				})->orWhereHas('buyer', function ($q) use ($query) {
					$q->where('username', 'like', "%$query%")
						->orWhere('name', 'like', "%$query%");
				});
		}

		$data = $transactions->orderBy('id', 'desc')->paginate(20)->withQueryString();

		return view('admin.transactions', ['data' => $data, 'query' => $query]);
	}

	public function cancelTransaction($id)
	{
		$transaction = Transactions::with(['creator:id,balance', 'buyer:id,wallet'])
			->whereId($id)
			->whereApproved('1')
			->firstOrFail();
		...
		if ($transaction->type == 'live_streaming_private') {
			LiveStreamingPrivateRequest::where('transaction_id', $transaction->id)->delete();
		}

		if ($transaction->type == 'purchase') {
			return self::salesRefund(Purchases::where('transactions_id', $transaction->id)->pluck('id')->first());
		} else {
			return self::refundTransaction($transaction);
		}
	}

	public function refundTransaction(Transactions $transaction)
	{
		try {
			$transaction->load(['creator:id,balance', 'buyer:id,wallet']);
			...
			// Total paid by buyer
			$amountRefund = number_format($amount + $totalTaxes, 2, '.', '');
			...
			// User Balnce Current
			$userBalance = $transaction->creator->balance;

			// If the creator has withdrawn their entire balance remove from withdrawal
			$withdrawalPending = Withdrawals::whereUserId($transaction->creator->id)->whereStatus('pending')->first();

			// Remove creator funds
			if ($userBalance <> 0.00) {
				$transaction->creator->decrement('balance', $transaction->earning_net_user);
			...
			$transaction->buyer->increment('wallet', $amountRefund);

			$transaction->delete();
		} catch (\Exception $e) {
			return redirect('panel/admin/transactions')->withErrorMessage($e->getMessage());
		}
	...
	public function savePayments(Request $request)
	{
		...
		$this->updatePaymentGatewayCreator($request);

		return redirect('panel/admin/payments')->withSuccessMessage(__('admin.success_update'));
	}

	public function updatePaymentGatewayCreator(Request $request)
	{
		$paymentMethods = [
			'payout_method_paypal' => 'PayPal',
			'payout_method_payoneer' => 'Payoneer',
			'payout_method_zelle' => 'Zelle',
			'payout_method_western_union' => 'Western Union',
			'payout_method_bank' => 'Bank',
			'payout_method_crypto' => 'Bitcoin',
			'payout_method_mercadopago' => 'Mercado Pago'
		];

		foreach ($paymentMethods as $method => $gateway) {
			if ($request->$method === 'off') {
				User::where('payment_gateway', $gateway)->update(['payment_gateway' => '']);
			}
		}
	}
	...
	public function withdrawals()
	{
		...
		if (strlen($query) >= 2) {
			$withdrawals->whereHas('creator', function ($q) use ($query) {
				$q->where('username', 'like', "%$query%")
					->orWhere('name', 'like', "%$query%");
			});
		}

		$data = $withdrawals->orderBy('id', 'desc')->paginate(30)->withQueryString();

		return view('admin.withdrawals', ['data' => $data, 'query' => $query]);
	}
	...
	public function posts()
	{
		...
		$data = Updates::when($sort == 'pending', function ($query) {
			$query->whereStatus('pending');
		})
			->when(strlen($query) >= 1, function ($q) use ($query) {
				if (is_numeric($query)) {
		...
		return view('admin.posts', ['data' => $data, 'sort' => $sort, 'query' => $query]);
	}
	...
	public function savePaymentsGateways($id, Request $request)
	{
		...
		// Coinpayments Legacy
		if (!$request->coinpayments_legacy) {
			$input['coinpayments_legacy'] = false;
		}
		...
		// Set AppID on .env file
		if ($data->name == 'OpenPix') {
			Helper::envUpdate('OPENPIX_APP_ID', ' "' . $input['key'] . '" ', true);
		}
	...
	public function themeStore(Request $request)
	{
		...
		if ($request->hasFile('logo_2')) {
			$extension = $request->file('logo_2')->extension();
		...
		if ($request->hasFile('watermak_video')) {
			$extension = $request->file('watermak_video')->extension();
		...
		if ($request->hasFile('index_image_top')) {
			...
			$extension  = $request->file('index_image_top')->extension();
		...
		if ($request->hasFile('image_index_1')) {
			...
			$extension  = $request->file('image_index_1')->extension();
		...
		if ($request->hasFile('image_index_2')) {
			...
			$extension  = $request->file('image_index_2')->extension();
		...
		if ($request->hasFile('image_index_3')) {
			...
			$extension  = $request->file('image_index_3')->extension();
		...
		if ($request->hasFile('image_index_4')) {
			...
			$extension  = $request->file('image_index_4')->extension();
		...
		if ($request->hasFile('avatar')) {
			...
			$imgAvatar = Image::read($request->file('avatar'))->cover(200, 200)->encodeByExtension($extension);
		...
		$this->settings->whereId(1)
			->update([
				...
				'theme_color_pwa' => $request->get('theme_color_pwa'),
	...
	public function memberVerification()
	{
		...
		$verificationRequests = VerificationRequests::query();

		if (strlen($query) >= 2) {
			$verificationRequests->whereHas('creator', function ($q) use ($query) {
				$q->where('username', 'like', "%$query%")
					->orWhere('name', 'like', "%$query%");
			});
		}

		$data = $verificationRequests->orderBy('id', 'desc')->paginate(30)->withQueryString();

		return view('admin.verification')->with(['data' => $data, 'query' => $query]);
	}
	...
	public function emailSettings(Request $request)
	{
		...
		foreach ($request->except(['_token']) as $key => $value) {
			Helper::updateEnvFile($key, $value);
		}
	...
	public function storage(Request $request)
	{
		...
		$request->validate([
			...
			'R2_ACCESS_KEY_ID' => 'required_if:FILESYSTEM_DRIVER,==,r2',
			'R2_SECRET_ACCESS_KEY' => 'required_if:FILESYSTEM_DRIVER,==,r2',
			'R2_BUCKET' => 'required_if:FILESYSTEM_DRIVER,==,r2',
			'R2_ENDPOINT' => 'required_if:FILESYSTEM_DRIVER,==,r2',

			'IDRIVE_KEY' => 'required_if:FILESYSTEM_DRIVER,==,idrive',
			'IDRIVE_SECRET' => 'required_if:FILESYSTEM_DRIVER,==,idrive',
			'IDRIVE_REGION' => 'required_if:FILESYSTEM_DRIVER,==,idrive',
	...
	public function createBlogStore(Request $request)
	{
		...
		if ($request->hasFile('thumbnail')) {
			...
			$imageResize = Image::read($image)->scale(width: 650)->encodeByExtension($extension);
	...
	public function deposits(Request $request)
	{
		...
		if ($query != '' && strlen($query) > 2) {
			$data = Deposits::where('txn_id', 'LIKE', '%' . $query . '%')->where('status', '!=', 'initialized')->orderBy('id', 'DESC')->paginate(30);
		} else {
			$data = Deposits::orderBy('id', 'DESC')->where('status', '!=', 'initialized')->paginate(30);
		}
	...
	public function saveLiveStreaming(Request $request)
	{
		...
		$this->settings->primary_certificate_agora    = $request->primary_certificate_agora;
	...
	{
		return view('admin.stories-posts', ['data' => Stories::has('media')->latest()->paginate(20)]);
	}
	...
	public function withdrawalsReject(Request $request)
	{
		$request->validate([
			'reason' => 'required|max:255'
		]);

		$withdrawal = Withdrawals::with(['creator:id,name,email,language,balance'])->findOrFail($request->id);

		$titleSite = config('settings.title');
		$emailUser = $withdrawal->creator->email;
		$sender = config('settings.email_no_reply');

		try {
			Mail::send(
				'emails.withdrawal-reject',
				[
					'body' => $request->reason,
					'title_site' => $titleSite,
					'fullname'   => $withdrawal->creator->name
				],
				function ($message) use ($sender, $titleSite, $emailUser) {
					$message->from($sender)
						->to($emailUser)
			...
			$withdrawal->creator->increment('balance', $withdrawal->amount);
	...
	public function saveVideoCallSettings()
	{
		$this->settings->agora_app_id = request('agora_app_id');
		$this->settings->primary_certificate_agora = request('primary_certificate_agora');
		$this->settings->video_call_status = request('video_call_status');
		$this->settings->video_call_max_price = request('video_call_max_price');
		$this->settings->video_call_min_price = request('video_call_min_price');
		$this->settings->video_call_max_duration = request('video_call_max_duration');
		$this->settings->save();

		return redirect('panel/admin/video-calls')->withSuccessMessage(__('admin.success_update'));
	}
	...
	public function addLikesExtras()
	{
		$likes = request('likes');
		$post = Updates::findOrFail(request('id'));
		$post->likes_extras += $likes;
		$post->save();

		return back()->withSuccessMessage(__('general.likes_added_successfully'));
	}

	public function editDatePost()
	{
		$post = Updates::findOrFail(request('id'));
		$post->date = now()->subDays(request('days'));
		$post->save();

		return back()->withSuccessMessage(__('general.edit_date_success'));
	}

	public function namesReserved()
	{
		$reserveds = DB::table('reserved')->orderBy('id', 'desc')->paginate(20);

		return view('admin.names-reserved', ['reserveds' => $reserveds]);
	}

	public function storeNameReserved(Request $request)
	{
		$request->validate([
			'name' => 'required|unique:reserved,name'
		]);
		...
		return back()->withSuccessMessage(__('general.success_added'));
	}

	public function deleteReserved($id)
	{
		DB::table('reserved')->where('id', $id)->where('editable', 1)->delete();

		return back()->withSuccessMessage(__('general.success_removed'));
	}

	public function saveAudioCallSettings()
	{
		$this->settings->audio_call_status = request('audio_call_status');
		$this->settings->primary_certificate_agora = request('primary_certificate_agora');
		$this->settings->agora_app_id = request('agora_app_id');
		$this->settings->audio_call_min_price = request('audio_call_min_price');
		$this->settings->audio_call_max_price = request('audio_call_max_price');
		$this->settings->audio_call_max_duration = request('audio_call_max_duration');
		$this->settings->save();

		return back()->withSuccessMessage(__('admin.success_update'));
	}

	public function saveGiphyAPI()
	{
		$this->settings->giphy_status = request('giphy_status');
		$this->settings->giphy_api_key = request('giphy_api_key');
		$this->settings->save();
	...
	{
		$this->settings->moderation_status = request('moderation_status');
		$this->settings->sightengine_api_user = request('sightengine_api_user');
		$this->settings->sightengine_api_api_secret = request('sightengine_api_api_secret');
		$this->settings->save();
	...
	public function saveAgeVerification(Request $request)
	{
		if (isset($request->age_verification_countries)) {
			$ageVerificationCountries = implode(',', $request->age_verification_countries);
		}

		$this->settings->age_verification_status = request('age_verification_status');
		$this->settings->age_verification_provider = request('age_verification_provider');
		$this->settings->show_modal_age_verification = request('show_modal_age_verification');
		$this->settings->threshold_age_verification = request('threshold_age_verification');
		$this->settings->age_verification_yoti_api_key = request('age_verification_yoti_api_key');
		$this->settings->age_verification_yoti_sdk_id = request('age_verification_yoti_sdk_id');
		$this->settings->age_verification_didit_api_key = request('age_verification_didit_api_key');
		$this->settings->age_verification_didit_webhook_secret = request('age_verification_didit_webhook_secret');
		$this->settings->age_verification_didit_workflow_id = request('age_verification_didit_workflow_id');
		$this->settings->age_verification_countries = $ageVerificationCountries ?? null;
		$this->settings->save();
