<header>
	<nav class="navbar navbar-expand-lg navbar-inverse fixed-top p-nav @if(auth()->guest() && request()->path() == '/' && $settings->home_style == 0) scroll @else p-3 @if (request()->is('live/*')) d-none @endif  @if (request()->is('messages/*')) d-none d-lg-block shadow-sm @elseif(request()->is('messages')) shadow-sm @else shadow-custom @endif {{ auth()->check() && auth()->user()->dark_mode == 'on' ? 'bg-white' : 'navbar_background_color' }} link-scroll @endif">
		<div class="container-fluid d-flex position-relative" style="max-width: 1280px; margin: 0 auto; height: 72px; align-items: center; justify-content: space-between;">
			...
			<a class="navbar-brand" href="{{url('/')}}">
				<img src="{{url('public/fans-foloow-me-logo-final-file--png-version.png')}}" alt="{{$settings->title}}" class="logo align-bottom max-w-100" />
			</a>
			...
			<div class="collapse navbar-collapse navbar-mobile" id="navbarCollapse">
				...
				@if (auth()->guest() && $settings->who_can_see_content == 'all' || auth()->check())
					<ul class="navbar-nav mr-auto" style="display: flex; align-items: center; gap: 0.25rem;">

						@guest
						{{-- fansfollowme.com navigation links --}}
						@if (!$settings->disable_creators_section)
						<li class="nav-item">
							<a class="nav-link font-weight-semibold" href="{{url('creators')}}">For Creators</a>
						</li>
						@endif

						<li class="nav-item">
							<a class="nav-link font-weight-semibold" href="{{url('fans')}}">For Fans</a>
						</li>

						<li class="nav-item">
							<a class="nav-link font-weight-semibold" href="{{url('celebrities')}}">Celebrities</a>
						</li>

						@if (!$settings->disable_creators_section)
						<li class="nav-item">
							<a class="nav-link font-weight-semibold" href="{{url('creators')}}">Explore</a>
						</li>
						@endif

						{{-- More dropdown --}}
						<li class="nav-item dropdown">
							<a class="nav-link font-weight-semibold dropdown-toggle" href="#" id="moreDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
								More <i class="feather icon-chevron-down" style="font-size: 0.75rem;"></i>
							</a>
							<div class="dropdown-menu dd-menu-user" aria-labelledby="moreDropdown">
								<a class="dropdown-item dropdown-navbar" href="{{url('casting')}}">🎬 Movie Casting</a>
								@if ($settings->live_streaming_status == 'on')
								<a class="dropdown-item dropdown-navbar" href="{{url('live')}}">🔴 Live Streams</a>
								@endif
								<a class="dropdown-item dropdown-navbar" href="{{url('business')}}">💼 Business</a>
								<a class="dropdown-item dropdown-navbar" href="{{url('scan')}}">📱 Scan Creator Code</a>
								@if (!$settings->disable_contact)
								<a class="dropdown-item dropdown-navbar" href="{{url('contact')}}">💬 Support</a>
								@endif
							</div>
						</li>
						@endguest
						...
						@if (!$settings->disable_creators_section)
							...
							<form class="form-inline my-lg-0 position-relative" method="get" action="{{url('creators')}}">
								<input id="searchCreatorNavbar"
									class="form-control search-bar border-0" type="text"
				...
				@endif
					...
					<ul class="navbar-nav ml-auto" style="display: flex; align-items: center; gap: 0.5rem;">
						@guest
						<li class="nav-item">
							<a @if (Helper::showLoginFormModal()) data-toggle="modal" data-target="#loginFormModal" @endif class="nav-link login-btn btn" href="{{ in_array(config('settings.home_style'), [0, 2]) ? url('login') : url('/')}}">
								{{__('auth.login')}}
							</a>
						</li>
						...
						<li class="nav-item">
							<a @if (Helper::showLoginFormModal()) data-toggle="modal" data-target="#loginFormModal" @endif class="toggleRegister nav-link btn btn-main btn-register-menu pr-3 pl-3 btn-arrow btn-arrow-sm" href="{{ in_array(config('settings.home_style'), [0, 2]) ? url('signup') : url('/') }}">
								Get Started
							</a>
						</li>
					@endif
			...
			<!-- =========== End Menu Mobile ============-->
				...
				<li class="nav-item dropdown d-lg-block d-none">
					...
					@endif
						...
						@if($settings->allow_vault)
							<a class="dropdown-item dropdown-navbar" href="{{url('my/vault')}}"><i class="feather icon-archive mr-2"></i> {{__('general.vault')}}</a>
						@endif
					@endif
