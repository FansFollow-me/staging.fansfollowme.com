...
// Global variables
  ...
  var sessionActive = {{ auth()->check() ? 'true' : 'false' }};
...
@auth
  ...
  var audioStatus = @if ($settings->disable_audio) [] @else ["audio/x-matroska","audio/mpeg"] @endif;
  @if ($settings->video_encoding == 'off')
  var extensionsPostMessage = ['png','jpeg','jpg','gif','ief','heic','heif','video/mp4',...audioStatus];
  var extensionsStories = ['png','jpeg','jpg','gif','ief','heic','heif','video/mp4'];
  var extensionsReels = ['video/mp4'];
  @else
  var extensionsPostMessage = ['png','jpeg','jpg','gif','ief','heic','heif','video/mp4','video/quicktime','video/3gpp','video/mpeg','video/x-matroska','video/x-ms-wmv','video/vnd.avi','video/avi','video/x-flv',...audioStatus];
  var extensionsStories = ['png','jpeg','jpg','gif','ief','heic','heif','video/mp4','video/quicktime','video/3gpp','video/mpeg','video/x-matroska','video/x-ms-wmv','video/vnd.avi','video/avi','video/x-flv'];
  ...
  var rejectedAudioCall = "{{ __('general.rejected_audio_call') }}";
  var rejectedAudioCallInfo = "{{ __('general.rejected_audio_call_info') }}";
  var unansweredAudioCall = "{{ __('general.audio_call_unanswered') }}";
  var unansweredAudioCallInfo = "{{ __('general.audio_call_unanswered_info') }}";
  ...
  var add = "{{ __('general.add') }}";
  var maximumFilesMsgError = "{{ __('general.files_can_be_selected_msg') }}";
  var information = "{{ __('general.information') }}";
  var followingFilesAlreadyAdded = "{{ __('general.following_files_already_added') }}";
  var newFilesHaveBeenAddedFollowingAlreadyExisted = "{{ __('general.new_files_have_been_added_following_already_existed') }}";
@endauth
</script>
...
border: 1px solid {{ auth()->check() && auth()->user()->dark_mode == 'on' ? '#5b5a5a' : '#ccc' }};
border-radius: 8px;
padding: 8px;
}
.progress-bar-custom {
background-color:{{ auth()->check() && auth()->user()->dark_mode == 'on' ? '#00aff01c' : '#00968845' }};
border-radius: 6px;
padding: 12px 16px;
color: {{ auth()->check() && auth()->user()->dark_mode == 'on' ? '#fff' : '#101010' }};
}
...
@else
.context-noty {color: {{ $settings->color_default }} !important;}
@endif
...
.tooltip {
  --bs-tooltip-bg: {{$settings->color_default}} !important;
...
.bs-tooltip-start .tooltip-arrow::before {
  border-left-color: {{$settings->color_default}} !important;
}
.bs-tooltip-end .tooltip-arrow::before {
  border-right-color: {{$settings->color_default}} !important;
}
.tooltip-inner {
  color: #fff !important;
  background-color: {{$settings->color_default}} !important;
...
.post-image-full {
  ...
  margin: 0 auto;
  display: block;
  background: @if (auth()->check() && auth()->user()->dark_mode == 'on') #2c2c2c @else #f8f8f8 @endif;
}
...
.modal textarea.textareaAutoSize {
