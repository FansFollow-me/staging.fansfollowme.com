  ...
  @foreach ($updates as $response)
	...
	<div class="media">
		...
		<div class="media-body">
				<h5 class="mb-0 font-montserrat text-break">
				...
				<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown_options">
					...
					@if ($response->crowdfund_goal != 0 && !$response->finalized)
					<button type="button" class="dropdown-item mb-1 actionFinalizeCampaign" data-post-id="{{$response->id}}" data-text="{{ __('donations.text_finalize_campaign') }}" data-confirm="{{ __('donations.yes_finalize_campaign') }}">
							<i class="bi-x-circle mr-2"></i> {{ __('donations.finalize_campaign') }}
						</button>
					@endif
					</div>
			@endif
...
@endif
		...
		@php 
			$response->insertPostView($response->id); 
		@endphp
...
@if ($response->status == 'active' && $response->crowdfund_goal != 0)
@php
	$fundsRaised = round(($response->funds_raised / $response->crowdfund_goal) * 100);
@endphp
<div class="container py-3">
        <div class="row justify-content-center">
            <div class="col-12">
			<div class="progress-container-donate mb-2">
				<div class="d-flex align-items-center justify-content-between">
					<div class="progress-bar-custom d-flex align-items-center gap-2 @if ($fundsRaised == 0) bg-transparent @endif" style="width: {{ $fundsRaised }}%;" id="progressBar{{ $response->id }}">
						<i class="bi bi-coin mr-2"></i>
						<span class="current-amount" id="currentAmount{{ $response->id }}">{{ Helper::amountFormat($response->funds_raised) }}</span>

						<div class="goal-section">
							<span>{{ Helper::amountFormat($response->crowdfund_goal) }}</span>

							@if ($fundsRaised != 0 && $response->creator->display_list_donors || $fundsRaised != 0 && auth()->check() && auth()->id() == $response->creator->id)
							<span class="c-pointer getDonors" data-id="{{ $response->id }}" data-toggle="modal" data-target="#getDonorsModal" title="{{__('donations.people_contributed')}}">
								<i class="bi-people-fill ml-2"></i>
							</span>
							@endif
						</div>
					</div>
				...
				@if (auth()->check() && auth()->id() != $response->creator->id && !$response->finalized)
				<button class="btn btn-donate text-uppercase my-3 w-100 py-2 rounded-pill" data-toggle="modal" data-target="#modalDonate" data-id="{{$response->id}}">
					{{ __('donations.donate') }}
				</button>

				@elseif ($response->finalized)
				<button class="btn btn-donate text-uppercase my-3 w-100 py-2 rounded-pill" disabled>
					{{ __('donations.campaign_ended') }}
				</button>

				@else
				<button class="btn btn-donate text-uppercase my-3 w-100 py-2 rounded-pill" id="btnDonateDisabled" @auth disabled @else data-toggle="modal" data-target="#loginFormModal" @endauth>
					{{ __('donations.donate') }}
				</button>
				@endif
			</div>
	...
	@endauth
		...
		<div class="w-100 mb-3 containerLikeComment">
			...
			<span class="text-muted dot-item">
				<i class="bi-play mr-1"></i> {{ Helper::formatNumber($response->video_views + $response->likes_extras) }}
			</span>
			@endif
		</div>
			...
			<div class="media-body">
				...
				<form action="{{url('comment/store')}}" method="post" @class(['comments-form', 'd-none' => !$response->creator->allow_comments])>
					...
					<div class="position-absolute" style="top: 40%; right: 0; display: block;">
						...
						<div class="dropdown">
							<span class="triggerGif" role="button" style="font-size:20px; position: absolute; right: 70px; cursor: pointer;" id="dropdownGif{{ $response->id }}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
								<i class="bi-filetype-gif"></i>
							</span>

							<div class="dropdown-menu dropdown-menu-right dropdown-emoji dropdown-gifs custom-scrollbar" aria-labelledby="dropdownGif{{ $response->id }}"></div>
						</div>
						@endif

						<div class="dropdown">
							<span class="triggerSticker" role="button" style="font-size:20px; position: absolute; right: 35px;  cursor: pointer;" id="dropdownSticky{{ $response->id }}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
								<i class="bi-sticky"></i>
							</span>

							<div class="dropdown-menu dropdown-menu-right dropdown-emoji dropdown-stickers custom-scrollbar" aria-labelledby="dropdownSticky{{ $response->id }}"></div>
						</div>
				...
				<input type="text" name="comment" class="form-control comments inputComment emojiArea border-0" autocomplete="off" placeholder="{{__('general.write_comment')}}" style="padding-right: 95px;">
			</div>
				</form>
			</div>
			@endif
