...
@section('content')
<div class="jumbotron jumbotron-cover-user home m-0 position-relative" style="padding: @if ($user->cover != '') @if (request()->path() == $user->username) 240px @else 125px @endif @else 125px @endif 0; background: #505050 @if ($user->cover != '') url('{{Helper::getFile(config('path.cover').$user->cover)}}') no-repeat center center !important; background-size: cover !important; @endif">
        ...
        <div class="w-100 text-center py-4 img-profile-user">
          ...
          <small class="text-muted font-14 d-block w-100 mt-1">
            ...
            @endif
              ...
              @elseif ($paymentGatewaySubscription == 'Flow' && $checkSubscription->cancelled == 'no')
              <form method="POST" action="{{ url('subscription/flow/cancel/'.$checkSubscription->id) }}" class="d-inline formCancel">
                @csrf
                <button type="button" data-expiration="{{ __('general.subscription_expire').' '.Helper::formatDate($checkSubscription->ends_at) }}" class="btn btn-success btn-profile mr-1 cancelBtn subscriptionActive">
                  <i class="feather icon-user-check mr-1"></i> {{__('general.your_subscribed')}}
                </button>
        ...
        <div class="modal-content">
          <div class="modal-body p-0">
            <div class="card bg-white shadow border-0">
              <div class="card-header pb-2 border-0 position-relative" style="height: 100px; background: {{$settings->color_default}} @if ($user->cover != '')  url('{{Helper::getFile(config('path.cover').$user->cover)}}') no-repeat center center @endif; background-size: cover; border-top-left-radius: 0.7rem;border-top-right-radius: 0.7rem;">
              ...
              <div class="card-body px-lg-5 py-lg-5 position-relative">
                ...
                <form method="post" action="{{url('buy/subscription')}}" id="formSubscription">
                  ...
                  @foreach ($allPayment as $payment)
                    ...
                    @if ($payment->name == 'Flow' && ! auth()->user()->flow_credit_card_type)
                      <div id="flowContainer" class="@if ($allPayment->count() == 1 && $payment->name == 'Flow')d-block @else display-none @endif">
                      <a href="{{ url('my/cards') }}" class="btn btn-secondary btn-sm mb-3 w-100">
                        <i class="far fa-credit-card mr-2"></i>
                        {{ __('general.add_payment_card') }}
                      </a>
