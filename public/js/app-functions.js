...
(function ($) {
	...
	// Bootstrap 4 to Bootstrap 5 Compatibility Bridge (MutationObserver and initial mapping)
	if (typeof MutationObserver !== 'undefined') {
		var bridgeAttributes = function (el) {
			...
			if (el.hasAttribute('data-slide') && !el.hasAttribute('data-bs-slide')) {
				el.setAttribute('data-bs-slide', el.getAttribute('data-slide'));
			}
			if (el.hasAttribute('data-slide-to') && !el.hasAttribute('data-bs-slide-to')) {
				el.setAttribute('data-bs-slide-to', el.getAttribute('data-slide-to'));
			}
			// Tooltip & Popover direct attributes
			var options = ['placement', 'html', 'content', 'trigger', 'delay', 'animation', 'container', 'template', 'offset', 'custom-class'];
			options.forEach(function (opt) {
				if (el.hasAttribute('data-' + opt) && !el.hasAttribute('data-bs-' + opt)) {
					el.setAttribute('data-bs-' + opt, el.getAttribute('data-' + opt));
				}
			});
		};
		...
		var observer = new MutationObserver(function (mutations) {
			mutations.forEach(function (mutation) {
				mutation.addedNodes.forEach(function (node) {
					if (node.nodeType === 1) {
						bridgeAttributes(node);
						node.querySelectorAll('[data-toggle], [data-target], [data-dismiss], [data-slide], [data-slide-to]').forEach(bridgeAttributes);
					}
				});
			});
		});
		...
		$(document).ready(function () {
			bridgeAttributes(document.body);
			document.querySelectorAll('[data-toggle], [data-target], [data-dismiss], [data-slide], [data-slide-to]').forEach(bridgeAttributes);
		});
	}
	...
	// jQuery Collapse Compatibility Bridge (Bootstrap 4.5.2 to Bootstrap 5.3.8)
	if (typeof bootstrap !== 'undefined' && bootstrap.Collapse && typeof bootstrap.Collapse.getInstance === 'function') {
	...
	// Intercept legacy click events for data-toggle="collapse"
	$(document).on('click', '[data-toggle="collapse"]', function (e) {
		...
		if (target && target !== '#') {
			...
			if ($target.length) {
				if (typeof bootstrap !== 'undefined' && bootstrap.Collapse && typeof bootstrap.Collapse.getInstance === 'function') {
					e.preventDefault();
					var instance = bootstrap.Collapse.getInstance($target[0]);
					if (!instance) {
						instance = new bootstrap.Collapse($target[0], { toggle: false });
					}
					instance.toggle();
				} else if (typeof $.fn.collapse === 'function') {
					e.preventDefault();
	...
	$(".navbar-toggler, .navbar-toggler-mobile").on('click', function () {
		$('.navbar-collapse, .collapsing').toggleClass('show');
		$('body').toggleClass("sidebar-overlay overflow-hidden");
	});

	// Close menu mobile
	$(".close-menu-mobile").on('click', function () {
		$('.navbar-collapse, .collapsing').removeClass('show');
	...
	// Finalize Campaign
	$(document).on('click', '.actionFinalizeCampaign', function (e) {
		e.preventDefault();

		var element = $(this);
		var textModal = element.attr('data-text');
		var textConfirm = element.attr('data-confirm');
			...
			function (isConfirm) {
				if (isConfirm) {
					$.ajax({
						...
						type: 'post',
						url: URL_BASE + '/finalize/campaign/' + postId
					}).done(function (data) {
						if (data.success) {
							$('#btnDonateDisabled').html(data.btnMessage);
							element.remove();
							$('.popout').removeClass('popout-error').addClass('popout-success').html('<i class="fa fa-check mr-1"></i> ' + data.message).slideDown('200').delay('3000').slideUp('50');
						}

					}).fail(function (jqXHR, ajaxOptions, thrownError) {
						$('.popout').removeClass('popout-success').addClass('popout-error').html(error_occurred + ' (' + thrownError + ')').slideDown('200').delay('5000').slideUp('50');
					});
				}
			});
	});
